<?php function PoschitatSchet($a, $b)
{
    ...
    $purch = "<div class=\"order322-line order322-title\">Счёт №$nu</div>";
    $syMMa = 0;
    foreach ( $a as $i )
    {
        $znach = $b[$i->id];
        ...
        {
            $sum = $znach * $i->price;
            $sum2 = number_format($i->price * $znach,2);
            $c =  number_format($i->price,2);
    ...
    $syMMa = number_format($syMMa,2);
    $purch .= "<div class=\"order322-total\"><div>Итого: $syMMa ₽</div></div>";
    return $purch;
}